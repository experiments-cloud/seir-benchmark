...
os.environ.setdefault("OUTPUT_DIR_OVERRIDE", "results_regime2")
...
DATA_FRACTIONS = [0.15, 0.45, 1.00]  # reduced set for time budget
N_SEEDS = 3  # reduced for time budget
N_COLLOCATION_POINTS = 800
N_WARMUP_ITERATIONS = 800
N_ADAM_ITERATIONS = 2000
N_LBFGS_ITERATIONS = 300
LEARNING_RATE = 1e-3
...
OUTPUT_DIR = "results_regime2"
os.makedirs(OUTPUT_DIR, exist_ok=True)
CHECKPOINT_PATH = os.path.join(OUTPUT_DIR, "einn_regime2_raw_results.csv")
...
def load_checkpoint():
    ...
    return pd.DataFrame(columns=["fraction","seed","beta_fitted","sigma_fitted","gamma_fitted","mean_rel_error"])
...
def main(max_fits_this_call=3):
    ...
    done_pairs = set(zip(checkpoint["fraction"], checkpoint["seed"])) if len(checkpoint) else set()

    all_jobs = [(frac, seed) for frac in DATA_FRACTIONS for seed in range(N_SEEDS)]
    n_done_this_call = 0
    for frac, seed in all_jobs:
        if (frac, seed) in done_pairs:
            continue
        if n_done_this_call >= max_fits_this_call:
            print(f"Reached this call's budget ({max_fits_this_call} fits); "
                  f"{len(done_pairs)+n_done_this_call}/{len(all_jobs)} total done. Re-run to continue.")
            return
        n_obs = max(int(np.floor(frac*len(time_grid))), 5)
        t_obs = time_grid[:n_obs]; i_obs = infectious[:n_obs]
        fitted = train_einn(t_obs, i_obs, full_duration, Y0, POPULATION_SIZE, compartment_scale, seed=RANDOM_SEED_BASE+seed)
        rel_err = np.abs(fitted-PARAMS_TRUE)/PARAMS_TRUE
        row = {"fraction":frac, "seed":seed, "beta_fitted":fitted[0], "sigma_fitted":fitted[1],
               "gamma_fitted":fitted[2], "mean_rel_error": float(np.mean(rel_err))}
        append_checkpoint(row)
        n_done_this_call += 1
        print(f"DONE fraction={frac} seed={seed}: mean_rel_error={row['mean_rel_error']:.4f} "
