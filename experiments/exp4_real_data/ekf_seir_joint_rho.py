...
rather than fixing it at an externally supplied value.

In the main EKF module, rho is fixed, so the observation model
z = rho * (H . x) is linear in the augmented state and a constant H
suffices. Once rho is itself a filtered state component, the observation
z = x_rho * (x_I + x_R) is a product of two state components and is no
longer linear, so this module replaces the fixed H with a nonlinear
observation function h(x), differentiated numerically the same way the
state-transition Jacobian already is in ekf_seir.py. The dynamics
themselves (SEIR plus the random-walk model for beta, sigma, gamma) are
unchanged; rho is simply appended to the state as an eighth component
evolving under the same random-walk assumption.
"""
...
def run_ekf_joint_rho(observations, dt, y0, param_init, rho_init, population,
    ...
    total_nis : float, cumulative normalized innovation squared over the
        whole fit, an internal goodness-of-fit criterion (lower is better)
        that does not require the true parameters, used to select among
        multiple random initializations exactly as NLS's final cost is
        used to select among its own random restarts.
    """
    ...
    total_nis = 0.0  # cumulative normalized innovation squared: an internal
                      # goodness-of-fit criterion that does not require
                      # knowledge of the true parameters, analogous to the
                      # final cost used to select among NLS's random restarts.
    ...
    for t in range(n_steps):
        ...
        total_nis += float((y_residual @ np.linalg.inv(S_innovation) @ y_residual.T).squeeze())
    ...
    param_estimate_final = np.concatenate([state_estimates[-1, 4:7],
                                            [state_estimates[-1, 7]]])
    return state_estimates, param_estimate_final, total_nis
