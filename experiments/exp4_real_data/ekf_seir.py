...
The state vector is augmented with the three epidemiological parameters,
modeled as constant in continuous time but assigned a small process noise
so the filter can adapt them as data arrives (parameters as augmented
state with random-walk process noise, a standard formulation for EKF
parameter estimation):

    x = [S, E, I, R, beta, sigma, gamma]

The state transition is the SEIR ODE for the first four components and a
random walk (identity map plus process noise) for the three parameter
components. Discretization uses fixed-step 4th-order Runge-Kutta between
observation times, since the observation interval (1 day) is coarse
relative to the epidemic's timescale and Euler discretization would
introduce non-negligible integration error.
...
linear in the state, so the observation Jacobian is a fixed selection and
scaling matrix.
"""
