...
that was noted explicitly as a limitation in the main analysis.

DESIGN NOTE ON THE RHO PRIOR
------------------------------
rho is a reporting fraction and can span several orders of magnitude
(from a few hundredths to a few ten-thousandths in this study's NLS
estimates), so a direct Uniform(0,1) prior would concentrate almost no
prior mass near the plausible region and would make sampling very
inefficient. Instead, a prior is placed on log(rho), Normal(log(0.01), 3),
weakly informative and spanning several orders of magnitude on either
side of 1%.

DESIGN NOTE ON NUMERICAL STABILITY
------------------------------------
The original fixed-rho script divided the observed data by rho to bring
it onto the same scale as the raw ODE solution. That is safe when rho is
a fixed constant, but unsafe when rho is itself a sampled random
variable that can take values close to zero, since dividing by a
near-zero sampled quantity produces extreme likelihood gradients and
destabilizes the sampler. This version instead multiplies the simulated
compartment trajectory by rho and compares directly to the (fixed-scale)
observed data, avoiding any division by a sampled quantity.

HOW TO RUN
-----------
    pip install -r requirements.txt
    python download_real_data.py    # if not already run
    python real_data_fit_mcmc_joint_rho.py
